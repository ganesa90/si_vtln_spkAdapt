function gmm = gmm_em(dataList, featDIR, nmix, final_niter, ds_factor, nworkers, gmmFilename)
...
if ischar(dataList) || iscellstr(dataList),
	dataList = load_data(dataList, featDIR);
end
if ~iscell(dataList),
...
function data = load_data(datalist, featDIR)
...
data = cell(nfiles, 1);
for ix = 1 : nfiles,
    data{ix} = htkread(strcat(featDIR, '/', filenames{ix},'.htk'));
end
